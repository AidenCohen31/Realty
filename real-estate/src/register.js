...
export default function Register() {
  ...
  async function handleSubmit(event){
    ...
    const resp = await axios.get("http://localhost:5000/email")
   ...
   const r = await axios.post("http://localhost:5000/register" , data)
        if(r.data.success){
  ...
  React.useEffect(() => {
    axios.get("http://localhost:5000/managed").then( (r) => {r.data.map((obj) => {setAddress((prev) => [...prev,obj.address])})})
  },[])  
